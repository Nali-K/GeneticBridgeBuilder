...
{
    public class BridgeStabilityDifferenceFitness:FitnessFunction
    {


        private IBridgeStabilitySimulation simulator;
        public BridgeStabilityDifferenceFitness(IBridgeStabilitySimulation simulator)
        {
        ...
        public override async Task<Dictionary<IChromosome,float>> CalculateFitnessAsync(List<IChromosome> chromosomes, CancellationToken token)
        {
            ...
            foreach (var chromosome in chromosomes)
            {
                    ...
                    outputDict.Add(chromosome,simulator.GetDifference(chromosome)*40f);
